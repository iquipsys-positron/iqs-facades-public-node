...
class SessionsOperationsV1 extends pip_services3_facade_node_1.FacadeOperations {
    constructor() {
        ...
        this._dependencyResolver.put('smssettings', new pip_services3_commons_node_3.Descriptor('pip-services-smssettings', 'client', '*', '*', '1.0'));
    ...
    openSession(req, res, account, roles) {
        ...
        let settings;
        async.series([
            // Retrieve organizations for user
            (callback) => {
                let organizationRoles = _.filter(roles, r => r.indexOf(':') > 0);
                let orgIds = _.map(organizationRoles, (r) => {
                    let pos = r.indexOf(':');
                    return pos >= 0 ? r.substring(0, pos) : r;
                });
                if (orgIds.length > 0) {
                    let filter = pip_services3_commons_node_2.FilterParams.fromTuples('ids', orgIds);
                    this._organizationsClient.getOrganizations(null, filter, null, (err, page) => {
                        organizations = page != null ? page.data : [];
                        callback(err);
                    });
                }
                else {
            ...
            (callback) => {
                let user = {
                    ...
                    theme: account.theme,
                    roles: roles,
                    organizations: _.map(organizations, s => { return { id: s.id, name: s.name }; }),
                    settings: settings,
                    ...
                    if (invitations) {
                        ...
                        for (let invitation of invitations) {
                            ...
                            if (invitation.org_id) {
                                ...
                                let role = invitation.org_id + ':' + invitation.role;
                                roles.push(role);
                            }
                        }
                    }
