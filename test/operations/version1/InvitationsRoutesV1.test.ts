...
suite('InvitationsOperationsV1', () => {
    ...
    test('should resend invitations', (done) => {
        ...
        // Send invitation
            (callback) => {
                rest.postAsUser(
                    ...
                    '/api/v1/organizations/' + INVITATION1.org_id + '/invitations',
                    INVITATION1,
        ...
        // Send another invitation
            (callback) => {
                rest.postAsUser(
                    TestUsers.AdminUserSessionId,
                    '/api/v1/organizations/' + invitation1.org_id + '/invitations/' + invitation1.id + '/resend', 
                    null,
                    (err, req, res, invitation) => {
                        assert.isNull(err);
    ...
    test('should perform invitation operations', (done) => {
        ...
        // Send another invitation
            (callback) => {
                rest.postAsUser(
                    TestUsers.AdminUserSessionId,
                    '/api/v1/organizations/' + INVITATION2.org_id + '/invitations', 
                    INVITATION2,
                    (err, req, res, invitation) => {
                        assert.isNull(err);
        ...
        // Delete invitation
            (callback) => {
                rest.delAsUser(
                    TestUsers.AdminUserSessionId,
                    '/api/v1/organizations/' + invitation1.org_id + '/invitations/' + invitation1.id,
                    (err, req, res, result) => {
                        assert.isNull(err);
    ...
    test('should notify invitations', (done) => {
        rest.postAsUser(
            TestUsers.AdminUserSessionId,
            '/api/v1/organizations/' + INVITATION1.org_id + '/invitations/notify',
            INVITATION3,
            (err, req, res) => {
                assert.isNull(err);
